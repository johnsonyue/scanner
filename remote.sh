...
pwd=$(pwd)
cd $cwd
out_file_name=$(echo $out_file | sed "s/^.*\///g")
...
echo "$iffinder -d -o $out_file_iffinder -c 200 -r 500 $cwd/$trace_ip_file"
...
ls $cwd/*iffinder.out | awk -F'/' '{print $NF}' | while read line; do tar zcvf $line.tar.gz $(echo $line | sed "s/^.*\///g"); done
cd $pwd
