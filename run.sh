...
[ -d $cwd/$date.run ] && echo "rm -rf $cwd/run-$date"
[ -d $cwd/$date.run ] && rm -rf $cwd/run-$date #delete pre-existing file.
echo "./midar-full --autostep --run-id=$date --dir=$cwd --mper-pps=100 --targets=$cwd/$trace_ip_file start"
./midar-full --autostep --run-id=$date --dir=$cwd --mper-port=$mper_port --mper-pps=100 --targets=$cwd/$trace_ip_file start
